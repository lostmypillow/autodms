...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (parseResult.success) {
            processedDocument = { ...processedDocument, ...rawDocument }
        ...
        console.log(`incoming category: ${processedDocument['category']}`)

        const checkOrderkeyResponse = await docClient.send(
            new QueryCommand({
                TableName: tableName,
                IndexName: 'SK-index',
                ...
                ExpressionAttributeValues: {
                    ':sk': `DATE#${today}`,
                    ':cat': processedDocument['category'],
                },
        ...
        const lastItem = checkOrderkeyResponse.Items?.[0]
        console.log(JSON.stringify(lastItem))
        const lastOrderKey = lastItem?.['orderKey'] ?? null
        console.log(`lastOrderKey: ${lastOrderKey}`)
        const generatedOrderKey = generateKeyBetween(lastOrderKey, null)
        console.log(`generatedOrderKey: ${generatedOrderKey}`)
        processedDocument = {
            ...processedDocument,
